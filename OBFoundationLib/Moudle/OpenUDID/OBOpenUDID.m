...
static NSString * kOBOpenUDIDSessionCache = nil;
//static NSString * const kOBOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const kOBOpenUDIDKey = @"OpenUDID";
static NSString * const kOBOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const kOBOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const kOBOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kOBOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kOBOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kOBOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kOBOpenUDIDRedundancySlots = 100;

@interface OBOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kOBOpenUDIDDomain];
#else
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forType:kOBOpenUDIDDomain];
#endif
}
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:kOBOpenUDIDDomain];
#else
	id item = [pboard dataForType:kOBOpenUDIDDomain];
#endif	
    if (item) {
        @try{
...
//
+ (NSString*) value {
    return [OBOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {

    if (kOBOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kOBOpenUDIDDomain
                                         code:kOBOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
        return kOBOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:kOBOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:kOBOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:kOBOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:kOBOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:kOBOpenUDIDOOTSKey];
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kOBOpenUDIDRedundancySlots];
    for (int n=0; n<kOBOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kOBOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [OBOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:kOBOpenUDIDKey];
            ...
            NSString* gid = [dict objectForKey:kOBOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:kOBOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;   
                }
            }
        }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [OBOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) { 
            ...
            [localDict setObject:openUDID forKey:kOBOpenUDIDKey];
            [localDict setObject:appUID forKey:kOBOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:kOBOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:kOBOpenUDIDTSKey];
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:kOBOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [OBOpenUDID _setDict:localDict forPasteboard:slotPB];
    }

    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:kOBOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kOBOpenUDIDDomain
                                                     code:kOBOpenUDIDErrorOptedOut
        ...
        kOBOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return kOBOpenUDIDSessionCache;
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kOBOpenUDIDDomain
                                         code:kOBOpenUDIDErrorCompromised
    ...
    kOBOpenUDIDSessionCache = [openUDID retain];
    return kOBOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:kOBOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:kOBOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:kOBOpenUDIDOOTSKey];

  	// store the dictionary locally
    [defaults setObject:dict forKey:kOBOpenUDIDKey];
    
    OpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
    kOBOpenUDIDSessionCache = nil;
